...
// Si l'une de ces variables est mal configurée, le createTransport peut échouer.
...
export async function POST(req: NextRequest) {
  let cvUrl = ""; // Déclaré ici pour être accessible dans le scope de la fonction

  try {
    // 1. TRAITEMENT DES DONNÉES ET FICHIER (Même logique - OK)
    ...
    if (!cvFile || cvFile.size === 0 || !cvFile.name) {
      return NextResponse.json(
        { success: false, message: "Le CV est manquant ou vide." },
        { status: 400 }
      );
    }
    const fileBuffer = await cvFile.arrayBuffer();
    const nodeBuffer = Buffer.from(fileBuffer); // --- 2. ISOLATION DE L'UPLOAD VERCEL BLOB ---

    try {
      const cvFileName = `${data.nom}_${data.prenom}_${Date.now()}_${
        cvFile.name
      }`;
      ...
      cvUrl = blob.url; // Assignation de l'URL
      console.log("Blob réussi. URL: ", cvUrl); // Log pour confirmation
    } catch (error) {
      console.error("ERREUR BLOB (CODE 500):", error); // Si le Blob échoue, arrêtez le traitement et renvoyez l'erreur
      return NextResponse.json(
        {
          success: false,
          message: `Échec de l'upload du CV. Vérifiez la clé BLOB_READ_WRITE_TOKEN. Détail: ${
            error instanceof Error ? error.message : "Inconnu"
          }`,
        },
        { status: 500 }
      );
    } // --- 3. ISOLATION DE L'ENREGISTREMENT POSTGRES ---
...
              );
            `;
      console.log("Postgres réussi."); // Log pour confirmation
    } catch (error) {
      console.error("ERREUR POSTGRES (CODE 500):", error);
      return NextResponse.json(
        {
          success: false,
          message: `Échec de l'enregistrement en BD. Vérifiez la clé POSTGRES_URL (mot de passe, sslmode). Détail: ${
            error instanceof Error ? error.message : "Inconnu"
          }`,
        },
        { status: 500 }
      );
    } // --- 4. ISOLATION DE L'ENVOI D'EMAIL ---
...
                `,
      ...
      console.log("SMTP réussi."); // Log pour confirmation
    } catch (error) {
      console.error("ERREUR SMTP (CODE 500):", error);
      return NextResponse.json(
        {
          success: false,
          message: `Échec de l'envoi d'email. Vérifiez les clés SMTP. Détail: ${
            error instanceof Error ? error.message : "Inconnu"
          }`,
        },
        { status: 500 }
    ...
    return NextResponse.json(
      {
        success: false,
        message: `Une erreur globale s'est produite. Détail: ${
          error instanceof Error ? error.message : "Inconnu"
        }`,
